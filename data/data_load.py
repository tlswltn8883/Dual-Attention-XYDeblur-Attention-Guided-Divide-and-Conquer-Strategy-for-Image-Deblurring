...
class DeblurDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image_path = os.path.join(self.image_dir, 'blur', image_name)

        label_base = os.path.splitext(image_name)[0]
        for ext in ['.jpg', '.png', '.jpeg']:
            label_path = os.path.join(self.image_dir, 'sharp', label_base + ext)
            if os.path.exists(label_path):
                break
        else:
            raise FileNotFoundError(f"No label file found for {image_name} in sharp folder.")
