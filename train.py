...
from utils import FFTLoss


def _train(model, config):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    criterion_l1 = torch.nn.L1Loss()
    criterion_fft = FFTLoss(loss_weight=0.1)
    ...
    for epoch_idx in range(start_epoch + 1, config.num_epoch + 1):
        ...
        for iter_idx, batch_data in enumerate(tqdm(dataloader)):
            ...
            l1_loss = criterion_l1(pred_img[config.num_subband], label_img)
            fft_loss = criterion_fft(pred_img[config.num_subband], label_img)
